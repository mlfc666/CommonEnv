...
import {DataSource} from "../config/DataSourceConfig.ts";
...
export default function MarkdownRender({content, fileServer = DataSource.MD_BASE,}: MarkdownRenderProps) {
    return (
            ...
            <ReactMarkdown
                ...
                components={{
                    ...
                    td: ({children, ...props}: ComponentPropsWithoutRef<'td'>) => (
                        <td className="min-w-25" {...props}>{children}</td>
                    ),
                    ...
                    code({inline, className, children, ...props}: CodeProps) {
                        ...
                        return (
                            <code
                                className="bg-base-200/80 px-1.5 py-0.5 rounded text-[0.85em] text-primary font-mono align-baseline font-medium" {...props}>
                                {children}
                            </code>
                        );
                    },
                    ...
                    a({href, children, ...props}: ComponentPropsWithoutRef<'a'>) {
                        ...
                        if (fileUrl.includes(fileServer) || fileUrl.endsWith('.zip') || fileUrl.endsWith('.rar')) {
                            return <FileCard href={fileUrl} fileName={linkText}/>;
                        }

                        return (
                            <a href={href}
                               className="link link-primary no-underline hover:underline font-medium transition-colors"
                    ...
                    img({src, alt, ...props}: ComponentPropsWithoutRef<'img'>) {
                        ...
                        return (
                            <figure className="my-8 flex flex-col items-center group">
                                ...
                                />
                                {alt && <figcaption
                                    className="text-center text-sm text-base-content/60 mt-3">{alt}</figcaption>}
                            </figure>
                        );
                    }
                }}

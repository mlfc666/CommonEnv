        ...
        const fetchTags = async () => {
            try {
                ...
                if (isMounted && res.code === 200) {
                    setAvailableTags(res.data.tags); // 取出 data 中的 tags 字段
                }
            } catch (error) {
        ...
        <div
            ...
            <div className="dropdown">
                <div tabIndex={0} role="button"
                    ...
                    <span
                        className="text-sm">{selectedTags.length > 0 ? `${t('sidebar.memo')} (${selectedTags.length})` : t('memo.filter.all_tags')}</span>
                ...
                <ul tabIndex={0}
                    ...
                    <li><label className="label cursor-pointer justify-start gap-3 py-2"><input type="checkbox"
                                                                                                className="checkbox checkbox-sm rounded"
                                                                                                checked={selectedTags.length === 0}
                                                                                                onChange={() => setSelectedTags([])}/><span
                        className={`label-text ${selectedTags.length === 0 ? 'font-bold' : ''}`}>{t('memo.filter.all_tags')}</span></label>
                    </li>
            ...
            <button onClick={() => onSearch(keyword, selectedTags, days)}

    ...
    return (
        <div className="drawer drawer-end text-base-content">
            ...
            <div className="drawer-content flex flex-col space-y-8">
                {/* 将刷新键传递给筛选条以同步更新标签池 */}
                <FilterBar onSearch={handleSearch} refreshKey={refreshKey} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    <button onClick={handleAddNew} className="group card border-2 border-dashed border-base-300 rounded-xl items-center justify-center text-base-content/20 hover:border-base-content hover:text-base-content transition-all min-h-65 bg-base-100/50">
                        ...
                        <span className="font-bold tracking-widest text-sm uppercase">{t('memo.page.new_btn')}</span>
                    </button>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mt-12 pt-8 border-t border-base-200 gap-6">
                    <div className="text-xs font-mono font-bold opacity-40 uppercase tracking-widest">
                        ...
                        <span className="text-base-content opacity-100 mx-1">{total === 0 ? 0 : startRecord} - {endRecord}</span>
                    ...
                    <div className="join border border-base-300 rounded-xl shadow-sm bg-base-100 overflow-hidden">
                        <button disabled={query.page === 1} onClick={() => setQuery(prev => ({ ...prev, page: prev.page - 1 }))} className="join-item btn btn-ghost btn-md px-8 text-xs uppercase tracking-widest border-r border-base-300">{t('memo.page.pagination_prev')}</button>
                        <button className="join-item btn btn-ghost btn-md px-6 font-black border-r border-base-300 no-animation">{query.page}</button>
                        <button disabled={query.page >= totalPages} onClick={() => setQuery(prev => ({ ...prev, page: prev.page + 1 }))} className="join-item btn btn-ghost btn-md px-8 text-xs uppercase tracking-widest border-l border-base-300">{t('memo.page.pagination_next')}</button>
                    </div>
                </div>
            </div>

            <MemoDrawer memo={editingMemo} setMemo={setEditingMemo} onSuccess={handleRefresh} />
        </div>
    );
}
...
import { UserIcon, KeyIcon, ArrowRightStartOnRectangleIcon } from "@heroicons/react/24/outline";
...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.SyntheticEvent<HTMLFormElement, SubmitEvent>) => {
        e.preventDefault();
        if (!cfToken) return alert(t('auth.login.cf_error'));
        ...
        <div className="min-h-[80vh] flex items-center justify-center">
            <div className="card w-full max-w-md bg-base-100 border border-base-300 rounded-xl shadow-none p-10">
                <div className="flex flex-col items-center mb-10 text-center">
                    ...
                    <p className="text-sm opacity-50 mt-2">{t('auth.login.subtitle')}</p>
                </div>

                <form onSubmit={handleLogin} className="space-y-6">
                    <fieldset className="fieldset p-0">
                        <legend className="fieldset-legend font-bold text-xs uppercase opacity-60">{t('auth.login.username_label')}</legend>
                        <label className="input input-bordered flex items-center gap-3 h-12 rounded-lg">
                            <UserIcon className="w-5 h-5 opacity-40" />
                            <input type="text" value={username} onChange={e => setUsername(e.target.value)} className="grow" placeholder={t('auth.login.username_placeholder')} required />
                        </label>
                    </fieldset>

                    <fieldset className="fieldset p-0">
                        <legend className="fieldset-legend font-bold text-xs uppercase opacity-60">{t('auth.login.password_label')}</legend>
                        <label className="input input-bordered flex items-center gap-3 h-12 rounded-lg">
                            <KeyIcon className="w-5 h-5 opacity-40" />
                            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="grow" placeholder={t('auth.login.password_placeholder')} required />
                        </label>
                    </fieldset>
                ...
                <div className="mt-8 text-center text-sm opacity-60">
                    {t('auth.login.no_account')} <Link to="/register" className="font-bold text-base-content hover:underline">{t('auth.login.register_link')}</Link>
                </div>
            </div>
        </div>
    );
